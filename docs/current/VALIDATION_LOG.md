...
- Magic Jewel full default command-probe sweep passed after the RuntimeEffect cache-gate calibration and focused
  ...
  `jbr_picture_frames=32385`, and `jbr_command_frames=183405`. The unsupported rows remain the intentional
  raw/unsupported shader, color-filter, path-effect, graphics-layer, saveLayer, and RuntimeEffect schema fallback
  sentinels. The TSV has 488 lines including the header. The run was 2.2G under Magic Jewel `out`, with `out` at 33G
  and the volume at about 387Gi free after completion. Suite:
  `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-command-probe-suite/20260527-023649/suite.tsv`.
- Magic Jewel compatibility matrix passed after the full command-probe invalid-refresh sweep. Aggregate: 57/57 passed,
  `fallback_sum=56`, `jbr_command_frames=431`, and `background_window=true` on every row. The only command frames
  came from the happy path; all ABI, native ABI, command-capability, high-capability, feature-capability, and public
  API mismatch rows fell back exactly once. The TSV has 58 lines including the header. The run was 86M under Magic
  Jewel `out`, with `out` at 29G and the volume at about 396Gi free after completion. Matrix:
...
- Magic Jewel artifact matrix passed on current ABI 106 local artifacts after the compatibility refresh. Required rows:
  `current-all` passed with no fallback and 463 JBR command frames, and `missing-public-api` passed with one expected
  public API fallback and zero command frames. The five optional old-artifact rows were recorded as skipped because no
  old bundle variables were configured. The TSV has 8 lines including the header. The run was 3.3M under Magic Jewel
  `out`, with `out` at 29G and the volume at about 396Gi free after completion. Matrix:
...
- Magic Jewel focused screenshot parity `CASE_GROUPS=descriptor-lifecycle` passed as the paired visual checkpoint for
  ...
  `jbr_picture_frames=0`, `jbr_command_frames=6153`, average pixel delta `2.127`, and average
...
- Magic Jewel focused command-probe `CASE_GROUPS=effect-descriptor-invalid` passed as the current effect descriptor
  ...
  path-effect descriptor payload guards. The run was 101M under Magic Jewel `out`, with `out` at 30G and the volume at
  about 393Gi free after completion. Suite:
...
- Magic Jewel focused command-probe `CASE_GROUPS=image-handles-invalid` passed as the current image handle/cache parser
  ...
  `jbr_command_frames=1162`. The run covered image define/cache-clear/evict record flags, image dimensions/pixel
  ...
  was 105M under Magic Jewel `out`, with `out` at 30G and the volume at about 393Gi free after completion. Suite:
...
- Magic Jewel focused command-probe `CASE_GROUPS=descriptor-handles-invalid` passed as the current descriptor handle
  ...
  210M under Magic Jewel `out`, with `out` at 30G and the volume at about 392Gi free after completion. Suite:
