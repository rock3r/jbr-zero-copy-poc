...
- Magic Jewel focused command-probe `CASE_GROUPS=gradient-path-invalid` passed as the current gradient path parser
  ...
  path-data length, path verb, and radial radius guards. The TSV has 19 lines including the header. The run was 61M
  under Magic Jewel `out`, with `out` at 47G and the volume at about 334Gi free after completion. Suite:
  `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-command-probe-suite/20260529-105239/suite.tsv`.
...
- Magic Jewel focused command-probe `CASE_GROUPS=shader-rendering` passed as the current shader-rendering command
  checkpoint. Aggregate: 13/13 passed, `fallback_sum=0`, `unsupported_rows=8`, `jbr_picture_frames=8059`, and
  `jbr_command_frames=5487`. It covered forced-context dynamic image-cache replay, image path-effect fallback, image
  ...
  48M under Magic Jewel `out`, with `out` at 47G and the volume at about 324Gi free after completion. Suite:
