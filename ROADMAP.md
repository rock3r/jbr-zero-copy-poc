...
- [ ] Generic shader strategy:
  ...
  - [x] JBR emits effect-handle use markers when a shader descriptor wraps a typed color-filter handle, and Magic Jewel asserts the composite shader + color-filter row consumes both shader and effect handles.
...
- [ ] Font/typeface ownership through the JBR Skia runtime.
  ...
  - [x] Added same-context resize and forced-context named system-font command/screenshot rows, proving concrete
    system-family native text remains command-backed after surface replacement and destination context migration:
    `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-command-probe-suite/20260504-111451/suite.tsv` and
...
- [ ] JBR-owned generic shader factory and handles for non-serialized shader families.
  ...
  - [x] Harden stable image/composite/linear-gradient shader+color-filter wrapper rows with shader cache-hit and
    nested effect-handle use gates. Focused command replay passed at
    `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-command-probe-suite/20260504-124614/suite.tsv`.
  - [x] Full default Magic Jewel command-probe suite passed after stable shader+color-filter wrapper gate tightening:
    `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-command-probe-suite/20260504-124837/suite.tsv`.
  - [x] Launch-level compatibility matrix passed after stable shader+color-filter wrapper gates and nested handle-use
    marker logging:
  ...
  - [x] Launch-level compatibility matrix passed after composite Perlin/noise shader metadata and lifecycle parity
    coverage:
    `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-compatibility-matrix/20260504-115559/matrix.tsv`.
    Happy stayed command-backed; all forced ABI/capability/public-API mismatch rows reported one structured fallback and
    zero JBR command frames, including the exact high-word `draw-points`, `font-data`, `shader-color`, and
    `shader-perlin-noise` rows.
  ...
  - [x] Added same-context resize and forced-context lifecycle command rows for composite Perlin/noise shader trees:
    ...
    change markers.
  - [x] Added same-context resize and forced-context screenshot parity rows for composite Perlin/noise shader trees:
    `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-screenshot-parity-suite/20260504-115247/suite.tsv`
    passed with zero fallback, zero JBR picture frames, positive command frames, command-cache clearing, surface/context
    change markers, and focused composite shader-region parity under the configured cap.
...
- [ ] Screen migration/context invalidation hardening beyond the current synthetic descriptor context-change probes.
  ...
  - [x] Add old/new screenshot parity rows for solid color shader descriptor redefinition across same-context resize and
    ...
    with zero fallback, zero picture frames, positive command frames, command-cache clearing, and second JBR
    shader-handle definitions.
  - [x] Full default Magic Jewel screenshot parity suite passed with the solid color shader migration rows included:
    `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-screenshot-parity-suite/20260504-021215/suite.tsv`.
    The run covered 56 rows, including Pulse/button chrome, resource/system font rows, point dots, shader migration
    rows, RuntimeEffect rows, and graphics-layer rows, all with zero fallback and zero JBR picture frames.
