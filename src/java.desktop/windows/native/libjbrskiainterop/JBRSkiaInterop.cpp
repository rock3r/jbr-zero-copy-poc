...
/*
 * Windows / Direct3D port of the JBR Skia command-replay backend.
 *
 * The command decoder (drawCommandList and every reconstruction helper) is
 * carried over verbatim from the macOS backend (JBRSkiaInterop.mm); only the
 * platform seams differ:
 *  - GPU context: a library-owned D3D12 device + queue feeding Skia Ganesh
 *    (GrDirectContexts::MakeD3D) instead of Java2D's MTLContext;
 *  - replay target: a shareable D3D12 render target handed to the Java2D
 *    Direct3D 9Ex pipeline through the Stage A sharing chain (shared fence ->
 *    D3D11.1 bridge -> legacy-shared texture); the Java service wraps the
 *    legacy texture via SharedTextures and blits it;
 *  - fonts: DirectWrite instead of CoreText.
 */
...
static constexpr const char* BUILD_ID = "skia=m147-64a2414108;flags=windows-release-d3d-poc:1;abi=111;native=3";
...
// ---------------------------------------------------------------------------
// Platform seam: forward declarations, DirectWrite fonts, diagnostics.
// The function name coreTextFontMgr() is kept so the decoder block matches
// the macOS backend byte-for-byte; on Windows it returns the DirectWrite
// font manager.
// ---------------------------------------------------------------------------

struct JBRSkiaD3DReplayContext;
...
static std::vector<SkString> commandFontFamilyCandidates(const std::string& fontFamily) {
    ...
    if (fontFamily == "sans-serif") {
        return {SkString("Segoe UI"), SkString("Tahoma"), SkString("Arial")};
    }
    if (fontFamily == "serif") {
        return {SkString("Times New Roman"), SkString("Georgia"), SkString("Cambria")};
    }
    if (fontFamily == "monospace") {
        return {SkString("Consolas"), SkString("Cascadia Mono"), SkString("Courier New")};
    }
    if (fontFamily == "cursive") {
        return {SkString("Segoe Script"), SkString("Comic Sans MS")};
    }
...
static size_t defaultMallocBytesInUse() {
    // Heap-usage sampling is a macOS diagnostic (malloc_zone_statistics);
    // not implemented on Windows — the malloc markers report deltas of 0.
    return 0;
}
...
// ---------------------------------------------------------------------------
// Direct3D replay context and targets.
//
// One library-owned D3D12 device/queue feeds Skia Ganesh. Replay targets are
// shareable D3D12 render textures; after replay the D3D11.1 bridge copies
// them into legacy-shared (D3D9Ex-openable) textures that the Java service
// wraps via the SharedTextures API and blits into the destination surface.
// Synchronization follows the Stage A A-corr contract: GPU-side fence
// ordering plus a bounded CPU drain of the single bridge copy.
// ---------------------------------------------------------------------------
...
struct JBRSkiaD3DReplayContext {
    ...
    ID3D12Fence* sharedFence12 = nullptr;
    ID3D11Fence* sharedFence11 = nullptr;
...
struct JBRSkiaD3DReplayTarget {
    int width = 0;
    int height = 0;
    ID3D12Resource* resource = nullptr;
    ID3D11Texture2D* opened11 = nullptr;
    ID3D11Texture2D* legacy11 = nullptr;

    ~JBRSkiaD3DReplayTarget() {
        if (legacy11) legacy11->Release();
        if (opened11) opened11->Release();
        if (resource) resource->Release();
    }
};

static std::mutex gReplayContextMutex;
static JBRSkiaD3DReplayContext* gReplayContext = nullptr;
static bool gReplayContextFailed = false;

static JBRSkiaD3DReplayContext* acquireReplayContext() {
    std::lock_guard<std::mutex> lock(gReplayContextMutex);
    if (gReplayContext != nullptr) {
        return gReplayContext;
    }
    if (gReplayContextFailed) {
        return nullptr;
    }

    auto fail = [](const char* stage) -> JBRSkiaD3DReplayContext* {
        std::fprintf(stderr, "JBR_SKIA_INTEROP_D3D_INIT status=failed stage=%s\n", stage);
        gReplayContextFailed = true;
        return nullptr;
    };

    std::unique_ptr<JBRSkiaD3DReplayContext> ctx(new JBRSkiaD3DReplayContext());

    IDXGIFactory4* factory = nullptr;
    if (FAILED(CreateDXGIFactory1(IID_PPV_ARGS(&factory)))) {
        return fail("dxgi-factory");
    }
    ...
    if (SUCCEEDED(hr)) {
        for (UINT i = 0;
             factory6->EnumAdapterByGpuPreference(i, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE,
                                                  IID_PPV_ARGS(&ctx->adapter)) != DXGI_ERROR_NOT_FOUND;
             ++i) {
            DXGI_ADAPTER_DESC1 desc = {};
            ctx->adapter->GetDesc1(&desc);
            if ((desc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE) == 0 &&
                SUCCEEDED(D3D12CreateDevice(ctx->adapter, D3D_FEATURE_LEVEL_11_0,
    ...
    if (FAILED(D3D12CreateDevice(ctx->adapter, D3D_FEATURE_LEVEL_11_0,
                                 IID_PPV_ARGS(&ctx->device12)))) {
        return fail("d3d12-device");
    }
    D3D12_COMMAND_QUEUE_DESC qd = {};
    qd.Type = D3D12_COMMAND_LIST_TYPE_DIRECT;
    if (FAILED(ctx->device12->CreateCommandQueue(&qd, IID_PPV_ARGS(&ctx->queue12)))) {
        return fail("d3d12-queue");
    }

    GrD3DBackendContext backendContext = {};
    backendContext.fAdapter.retain(ctx->adapter);
    backendContext.fDevice.retain(ctx->device12);
    backendContext.fQueue.retain(ctx->queue12);
    backendContext.fProtectedContext = GrProtected::kNo;
    ctx->directContext = GrDirectContexts::MakeD3D(backendContext);
    if (ctx->directContext == nullptr) {
        return fail("skia-direct-context");
    }
    ...
    ctx->hD3D11 = LoadLibraryW(L"d3d11.dll");
    FnD3D11CreateDevice createDevice11 = ctx->hD3D11 == nullptr ? nullptr :
        (FnD3D11CreateDevice)GetProcAddress(ctx->hD3D11, "D3D11CreateDevice");
    if (createDevice11 == nullptr) {
        return fail("d3d11-load");
    }
    D3D_FEATURE_LEVEL fl = D3D_FEATURE_LEVEL_11_1;
    if (FAILED(createDevice11(ctx->adapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, 0,
                              &fl, 1, D3D11_SDK_VERSION,
                              &ctx->dev11, nullptr, &ctx->ctx11)) ||
        FAILED(ctx->dev11->QueryInterface(IID_PPV_ARGS(&ctx->dev11_1))) ||
        FAILED(ctx->dev11->QueryInterface(IID_PPV_ARGS(&ctx->dev11_5))) ||
        FAILED(ctx->ctx11->QueryInterface(IID_PPV_ARGS(&ctx->ctx11_4)))) {
        return fail("d3d11-device");
    }

    HANDLE hFence = nullptr;
    if (FAILED(ctx->device12->CreateFence(0, D3D12_FENCE_FLAG_SHARED,
                                          IID_PPV_ARGS(&ctx->sharedFence12))) ||
        FAILED(ctx->device12->CreateSharedHandle(ctx->sharedFence12, nullptr, GENERIC_ALL,
                                                 nullptr, &hFence)) ||
        FAILED(ctx->dev11_5->OpenSharedFence(hFence, IID_PPV_ARGS(&ctx->sharedFence11)))) {
        if (hFence) CloseHandle(hFence);
        return fail("shared-fence");
    }
    CloseHandle(hFence);

    ctx->copyFenceEvent = CreateEventExW(nullptr, nullptr, 0, EVENT_ALL_ACCESS);
    if (ctx->copyFenceEvent == nullptr ||
        FAILED(ctx->dev11_5->CreateFence(0, D3D11_FENCE_FLAG_NONE,
                                         IID_PPV_ARGS(&ctx->copyFence11)))) {
    ...
    ctx->adapter->GetDesc1(&desc);
    std::fprintf(stderr,
                 "JBR_SKIA_INTEROP_D3D_INIT status=ok adapterLuid=%08lX-%08lX\n",
                 desc.AdapterLuid.HighPart, desc.AdapterLuid.LowPart);
    gReplayContext = ctx.release();
    return gReplayContext;
}

static JBRSkiaD3DReplayTarget* acquireReplayTarget(JBRSkiaD3DReplayContext* ctx,
                                                   JBRSkiaD3DReplayTarget* oldTarget,
                                                   int width, int height) {
    if (oldTarget != nullptr && oldTarget->width == width && oldTarget->height == height) {
        return oldTarget;
    }
    delete oldTarget;

    std::unique_ptr<JBRSkiaD3DReplayTarget> target(new JBRSkiaD3DReplayTarget());
    target->width = width;
    ...
    HANDLE hTex = nullptr;
    if (FAILED(ctx->device12->CreateCommittedResource(
            &heap, D3D12_HEAP_FLAG_SHARED, &td, D3D12_RESOURCE_STATE_COMMON,
            nullptr, IID_PPV_ARGS(&target->resource))) ||
        FAILED(ctx->device12->CreateSharedHandle(target->resource, nullptr, GENERIC_ALL,
                                                 nullptr, &hTex)) ||
        FAILED(ctx->dev11_1->OpenSharedResource1(hTex, IID_PPV_ARGS(&target->opened11)))) {
    ...
    ld.MipLevels = 1;
    ld.ArraySize = 1;
    ld.Format = DXGI_FORMAT_B8G8R8A8_UNORM;
    ld.SampleDesc.Count = 1;
    ld.Usage = D3D11_USAGE_DEFAULT;
    ld.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE;
    ld.MiscFlags = D3D11_RESOURCE_MISC_SHARED; // legacy handle for D3D9Ex
    if (FAILED(ctx->dev11->CreateTexture2D(&ld, nullptr, &target->legacy11))) {
        return nullptr;
    }
...
static sk_sp<SkSurface> wrapTextureSurface(GrDirectContext* directContext,
                                           JBRSkiaD3DReplayTarget* target) {
    GrD3DTextureResourceInfo texResInfo = {};
    texResInfo.fResource.retain(target->resource);
    ...
    GrBackendRenderTarget renderTarget =
        GrBackendRenderTargets::MakeD3D(target->width, target->height, texResInfo);
...
// Orders replay -> bridge copy -> bounded drain (A-corr); after this returns
// true the legacy texture holds the frame and the Java side may blit it.
static bool bridgeCopyAndDrain(JBRSkiaD3DReplayContext* ctx, JBRSkiaD3DReplayTarget* target) {
    ctx->fenceValue++;
    if (FAILED(ctx->queue12->Signal(ctx->sharedFence12, ctx->fenceValue))) {
        return false;
    }
    if (FAILED(ctx->ctx11_4->Wait(ctx->sharedFence11, ctx->fenceValue))) {
        return false;
    }
    ctx->ctx11->CopyResource(target->legacy11, target->opened11);
    ctx->ctx11->Flush();
    ctx->copyFenceValue++;
    if (FAILED(ctx->ctx11_4->Signal(ctx->copyFence11, ctx->copyFenceValue))) {
        return false;
    }
    if (ctx->copyFence11->GetCompletedValue() < ctx->copyFenceValue) {
        if (FAILED(ctx->copyFence11->SetEventOnCompletion(ctx->copyFenceValue,
                                                          ctx->copyFenceEvent))) {
            return false;
        }
        if (WaitForSingleObject(ctx->copyFenceEvent, 1000) != WAIT_OBJECT_0) {
            return false;
        }
    }
    return true;
...
// The recorded command stream is window-anchored (the Skiko layer rebases
// its graphics to window coordinates); the replay target covers only the
// destination clip, so the canvas is translated by the negative clip origin.
template <typename CommandWordsT>
static jboolean renderCommandsIntoTarget(jlong targetHandle,
                                         jint destinationX,
                                         jint destinationY,
                                         jfloat scaleX,
                                         jfloat scaleY,
                                         jint width,
                                         jint height,
                                         CommandWordsT commands,
                                         jsize commandCount,
                                         const char* frameTag) {
    JBRSkiaD3DReplayTarget* target =
        reinterpret_cast<JBRSkiaD3DReplayTarget*>(static_cast<uintptr_t>(targetHandle));
    if (target == nullptr || width <= 0 || height <= 0 || commandCount <= 0) {
        return JNI_FALSE;
    }
    JBRSkiaD3DReplayContext* ctx = acquireReplayContext();
    if (ctx == nullptr) {
        return JNI_FALSE;
    }

    const long long frameStartNanos = monotonicNanos();
    sk_sp<SkSurface> surface = wrapTextureSurface(ctx->directContext.get(), target);
    if (surface == nullptr) {
        return JNI_FALSE;
    }
    ...
    if (scaleX > 0.0f && scaleY > 0.0f && (scaleX != 1.0f || scaleY != 1.0f)) {
        // command streams are recorded in user (logical) space
        canvas->scale(scaleX, scaleY);
    }
    bool rendered = drawCommandList(canvas, commands, commandCount, width, height,
                                    static_cast<void*>(ctx), &metrics);
    canvas->restore();
    ...
    ctx->directContext->flushAndSubmit(surface.get(), GrSyncCpu::kNo);
    if (!bridgeCopyAndDrain(ctx, target)) {
        return JNI_FALSE;
    }
    const long long flushNanos = monotonicNanos() - flushStartNanos;
    purgeUnlockedResourcesAfterFrame(ctx->directContext.get());
    std::fprintf(stderr,
                 "JBR_SKIA_INTEROP_COMMAND_FRAME destinationX=0 destinationY=0 destinationWidth=%d destinationHeight=%d width=%d height=%d commands=%d rendered=true backend=%s\n",
                 target->width,
                 target->height,
                 width,
                 height,
                 static_cast<int>(commandCount),
                 frameTag);
    std::fprintf(stderr,
...
// ---------------------------------------------------------------------------
// JNI surface (Windows service contract).
// ---------------------------------------------------------------------------
...
Java_com_jetbrains_desktop_JBRSkiaService_nativeGetContextId
    ...
    JBRSkiaD3DReplayContext* ctx = acquireReplayContext();
    return ctx == nullptr ? 0 : static_cast<jlong>(reinterpret_cast<uintptr_t>(ctx));
}
...
extern "C" JNIEXPORT jlong JNICALL
Java_com_jetbrains_desktop_JBRSkiaService_nativeAcquireReplayTarget
        (JNIEnv* env, jclass cls, jlong oldTargetHandle, jint width, jint height) {
    if (width <= 0 || height <= 0) {
        return 0;
    }
    JBRSkiaD3DReplayContext* ctx = acquireReplayContext();
    ...
    JBRSkiaD3DReplayTarget* oldTarget =
        reinterpret_cast<JBRSkiaD3DReplayTarget*>(static_cast<uintptr_t>(oldTargetHandle));
    JBRSkiaD3DReplayTarget* target = acquireReplayTarget(ctx, oldTarget, width, height);
    return static_cast<jlong>(reinterpret_cast<uintptr_t>(target));
}

extern "C" JNIEXPORT void JNICALL
Java_com_jetbrains_desktop_JBRSkiaService_nativeDisposeReplayTarget
        (JNIEnv* env, jclass cls, jlong targetHandle) {
    delete reinterpret_cast<JBRSkiaD3DReplayTarget*>(static_cast<uintptr_t>(targetHandle));
}

extern "C" JNIEXPORT jlong JNICALL
Java_com_jetbrains_desktop_JBRSkiaService_nativeGetReplayLegacyTexturePtr
        (JNIEnv* env, jclass cls, jlong targetHandle) {
    JBRSkiaD3DReplayTarget* target =
        reinterpret_cast<JBRSkiaD3DReplayTarget*>(static_cast<uintptr_t>(targetHandle));
    return target == nullptr ? 0
                             : static_cast<jlong>(reinterpret_cast<uintptr_t>(target->legacy11));
}
...
Java_com_jetbrains_desktop_JBRSkiaService_nativeRenderDiagnosticFrame
    ...
    drawDiagnosticPattern(surface->getCanvas(), target->width, target->height, frameTimeNanos);
    ctx->directContext->flushAndSubmit(surface.get(), GrSyncCpu::kNo);
    return bridgeCopyAndDrain(ctx, target) ? JNI_TRUE : JNI_FALSE;
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_jetbrains_desktop_JBRSkiaService_nativeRenderPictureFrame
        (JNIEnv* env, jclass cls, jlong targetHandle,
         jint destinationX, jint destinationY,
         jfloat scaleX, jfloat scaleY,
         jint width, jint height, jlong frameTimeNanos, jbyteArray pictureArray) {
    JBRSkiaD3DReplayTarget* target =
        reinterpret_cast<JBRSkiaD3DReplayTarget*>(static_cast<uintptr_t>(targetHandle));
    if (target == nullptr || width <= 0 || height <= 0 || pictureArray == nullptr) {
        return JNI_FALSE;
    }
    JBRSkiaD3DReplayContext* ctx = acquireReplayContext();
    if (ctx == nullptr) {
    ...
    std::fprintf(stderr,
                 "JBR_SKIA_INTEROP_PICTURE_FRAME destinationX=0 destinationY=0 destinationWidth=%d destinationHeight=%d width=%d height=%d bytes=%d rendered=true\n",
                 target->width, target->height, width, height, pictureSize);
    return JNI_TRUE;
}
...
Java_com_jetbrains_desktop_JBRSkiaService_nativeRenderCommandFrame
    ...
    jboolean rendered = renderCommandsIntoTarget(targetHandle, destinationX, destinationY,
                                                 scaleX, scaleY,
                                                 width, height,
                                                 IntCommandWords{commands}, commandCount,
                                                 "d3d-int");
...
Java_com_jetbrains_desktop_JBRSkiaService_nativeRenderCommandBufferFrame
    ...
    jsize byteCount = env->GetArrayLength(commandArray);
    if (byteCount <= 0 || byteCount % static_cast<jsize>(sizeof(jint)) != 0) {
        return JNI_FALSE;
    }
    jboolean isCopy = JNI_FALSE;
    ...
    jboolean rendered = renderCommandsIntoTarget(targetHandle, destinationX, destinationY,
                                                 scaleX, scaleY,
                                                 width, height,
                                                 LittleEndianByteCommandWords{commandBytes},
                                                 byteCount / static_cast<jsize>(sizeof(jint)),
                                                 "d3d-buffer");
...
Java_com_jetbrains_desktop_JBRSkiaService_nativeRenderCommandDirectFrame
        (JNIEnv* env, jclass cls, jlong targetHandle,
         jint destinationX, jint destinationY,
         jfloat scaleX, jfloat scaleY,
         jint width, jint height, jlong frameTimeNanos, jobject commandBuffer, jint byteCount) {
    if (commandBuffer == nullptr || byteCount <= 0 ||
            byteCount % static_cast<jint>(sizeof(jint)) != 0) {
        return JNI_FALSE;
    }
    ...
    return renderCommandsIntoTarget(targetHandle, destinationX, destinationY,
                                                 scaleX, scaleY,
                                                 width, height,
                                    LittleEndianByteCommandWords{commandBytes},
                                    byteCount / static_cast<jint>(sizeof(jint)),
                                    "d3d-direct");
}
