...
public class JBRSkiaService extends JBRSkia {
    ...
    public static boolean isValidCommandStreamForTesting(int[] commands) {
        ...
        while (offset < commandEnd) {
            ...
            if (record.op() == COMMAND_CLEAR_IMAGE_CACHE) {
                ...
                return false;
            } else if (record.op() == COMMAND_SAVE_TRANSLATE_LAYER_SAVE_TRANSLATE_DRAW_IMAGE_REF_FULL_RESTORE_N
                    && !imageDimensions.containsKey(commandHandle(
                    commands[record.argsStart() + 13],
                ...
                return false;
            } else if (record.op() == COMMAND_CLEAR_DRAW_IMAGE_REF_FULL_DRAW_ROUND_RECT
                    && !imageDimensions.containsKey(commandHandle(
                    commands[record.argsStart() + 9],
    ...
    private static int expectedRecordLength(int op) {
        ...
        if (op == COMMAND_CLEAR_DRAW_IMAGE_REF_FULL_DRAW_ROUND_RECT) return 26;
        ...
        if (op == COMMAND_SAVE_TRANSLATE_LAYER_SAVE_TRANSLATE_DRAW_IMAGE_REF_FULL_RESTORE_N) return 19;
    ...
    private static boolean validateRecordArguments(int[] commands, CommandRecord record) {
        ...
        if (record.op() == COMMAND_SAVE_TRANSLATE_LAYER_SAVE_TRANSLATE_DRAW_IMAGE_REF_FULL_RESTORE_N
                && commands[record.recordEnd() - 1] <= 0) {
            return false;
        }
        ...
        if (record.op() == COMMAND_STROKE_CLOSED_POLYLINE) {
            if (record.recordFlags() != COMMAND_RECORD_FLAGS_NONE
                    && record.recordFlags() != COMMAND_RECORD_FLAG_ANTIALIAS) {
                return false;
            }
        ...
        if (record.op() == COMMAND_SAVE_TRANSLATE_ROTATE_TRANSLATE_FILL_OVAL_RESTORE_RUN) {
            ...
            if (ovalCount < 2
                    || ovalCount > 4096
                    || record.argsStart() + 1 + ovalCount * 10 != record.recordEnd()) {
                return false;
            }
            int offset = record.argsStart() + 1;
            for (int i = 0; i < ovalCount; i++) {
                int width = commands[offset + 8];
                int height = commands[offset + 9];
                if (width < 0 || height < 0) {
                    return false;
                }
                offset += 10;
        ...
        if (record.op() == COMMAND_DRAW_IMAGE_REF_FULL
                ...
                || record.op() == COMMAND_DRAW_IMAGE_REF_FULL_RESTORE_N
                || record.op() == COMMAND_DRAW_IMAGE_REF_FULL_RESTORE_N_SAVE_TRANSLATE_LAYER_SAVE_TRANSLATE
                || record.op() == COMMAND_SAVE_TRANSLATE_LAYER_SAVE_TRANSLATE_DRAW_IMAGE_REF_FULL_RESTORE_N) {
            if (record.recordFlags() != COMMAND_RECORD_FLAGS_NONE
                    && record.recordFlags() != COMMAND_RECORD_FLAG_ANTIALIAS) {
                return false;
            }
        ...
        if (record.op() == COMMAND_CLEAR_DRAW_IMAGE_REF_FULL_DRAW_ROUND_RECT) {
            if (record.recordFlags() != COMMAND_RECORD_FLAGS_NONE
                    && record.recordFlags() != COMMAND_RECORD_FLAG_ANTIALIAS) {
                return false;
            }
            int clearWidth = commands[record.argsStart() + 2];
            int clearHeight = commands[record.argsStart() + 3];
            int imageFlags = commands[record.argsStart() + 4];
            if (clearWidth < 0
                    || clearHeight < 0
                    || (imageFlags != COMMAND_RECORD_FLAGS_NONE && imageFlags != COMMAND_RECORD_FLAG_ANTIALIAS)) {
                return false;
            }
            int roundArgsStart = record.argsStart() + 11;
            int paintStyle = commands[roundArgsStart];
            int left1000 = commands[roundArgsStart + 2];
            int top1000 = commands[roundArgsStart + 3];
            int right1000 = commands[roundArgsStart + 4];
    ...
    private static final class PocScopedSkiaCanvas extends ScopedSkiaCanvas {
        // Windows/Direct3D presentation: the native backend replays into a
        // library-owned shareable D3D12 target; after the bridge copy the
        // legacy-shared consumer texture is wrapped once via SharedTextures
        // and blitted onto the destination Graphics2D. Single shared replay
        // target (matches the single-Compose-surface PoC scope); guarded by
        // REPLAY_LOCK.
        private static final Object REPLAY_LOCK = new Object();
        private static long replayTarget;
        private static long wrappedLegacyPtr;
        private static Image wrappedImage;
        private static GraphicsConfiguration wrappedConfiguration;
        private static SharedTextures sharedTextures;
        private static boolean sharedTexturesUnavailable;
        ...
        private PocScopedSkiaCanvas(long scopeId, Graphics2D graphics, Rectangle userSpaceClip) {
            ...
            this.contextPtr = NATIVE_BRIDGE_AVAILABLE ? nativeGetContextId(0) : 0;
        }

        private interface NativeFrameSender {
            boolean send(long replayTargetHandle, int replayOffsetX, int replayOffsetY,
                         float replayScaleX, float replayScaleY);
        }

        /**
         * Runs the native replay into the shared target and presents the
         * result through the Stage A hand-off. Returns false when any step
         * is unavailable so the caller can fall back.
         *
         * The target mirrors the macOS destination semantics: it is sized to
         * the destination surface (window) and persists across frames, so
         * incremental (dirty-region) command streams accumulate correctly;
         * only the destination clip subregion is blitted each frame. The
         * command streams are layer-local; the replay translate anchors
         * them at the layer's device origin on the target.
         */
        private boolean renderNativeAndPresent(NativeFrameSender sender) {
            if (!NATIVE_BRIDGE_AVAILABLE || deviceSpaceClip == null
                    || deviceSpaceClip.width <= 0 || deviceSpaceClip.height <= 0) {
                return false;
            }
            // Destination ground truth: the surface's device-space composite
            // clip (buffer-anchored). The recorded command streams are
            // layer-local, so the replay translate maps layer-local command
            // space onto the buffer-anchored target at the layer's device
            // origin (deviceSpaceClip).
            Rectangle surfaceBounds = null;
            Rectangle destDeviceClip = null;
            if (graphics instanceof SunGraphics2D sunGraphics) {
                surfaceBounds = sunGraphics.getSurfaceData().getBounds();
                sun.java2d.pipe.Region compClip = sunGraphics.getCompClip();
                if (compClip != null) {
                    destDeviceClip = new Rectangle(
                            compClip.getLoX(), compClip.getLoY(),
                            compClip.getWidth(), compClip.getHeight());
                }
            }
            if (surfaceBounds == null || surfaceBounds.width <= 0 || surfaceBounds.height <= 0
                    || destDeviceClip == null
                    || destDeviceClip.width <= 0 || destDeviceClip.height <= 0) {
                return false;
            }
            // The command streams are layer-local (recorded with the canvas
            // origin at the layer's top-left) and the scope's device-space
            // clip is the layer's bounds on the destination surface, so the
            // replay must place the stream at the layer's device origin:
            // native replay applies translate(-destinationX, -destinationY),
            // hence the negated origin. (The former deviceClip-destClip
            // difference was 0 for full-layer damage and anchored the stream
            // to the damage rect - not the layer - for partial damage.)
            final int replayOffsetX = -deviceSpaceClip.x;
            final int replayOffsetY = -deviceSpaceClip.y;
            // command streams arrive in device space: Compose applies the
            // density itself when drawing into the recorded Skiko canvas, so
            // replay is 1:1 even on HiDPI backing scales (a clip-ratio-derived
            // scale double-applies; verified b1-dir-C vs b1-dir-C2). The
            // native scale plumbing stays for recorders that emit logical space.
            final float replayScaleX = 1.0f;
            final float replayScaleY = 1.0f;
            final Rectangle blitRect = destDeviceClip;
            synchronized (REPLAY_LOCK) {
                long target = nativeAcquireReplayTarget(
                        replayTarget, surfaceBounds.width, surfaceBounds.height);
                if (target != replayTarget) {
                    // target was recreated: any cached wrapper is stale
                    wrappedLegacyPtr = 0;
                    wrappedImage = null;
                }
                replayTarget = target;
                ...
                if (!sender.send(target, replayOffsetX, replayOffsetY, replayScaleX, replayScaleY)) {
                    return false;
                }
                long legacyPtr = nativeGetReplayLegacyTexturePtr(target);
                if (legacyPtr == 0) {
                    return false;
                }
                Image image = wrapLegacyTexture(legacyPtr);
                if (image == null) {
                    return false;
                }
                // Blit only the destination device-clip subregion (buffer
                // coordinates), under the configuration's default transform.
                Graphics2D blitGraphics = (Graphics2D) graphics.create();
                try {
                    java.awt.geom.AffineTransform deviceTransform =
                            blitGraphics.getDeviceConfiguration().getDefaultTransform();
                    double scale = deviceTransform.getScaleX() != 0
                            ? deviceTransform.getScaleX() : 1.0;
                    blitGraphics.setTransform(deviceTransform);
                    int x1 = (int) Math.round(blitRect.x / scale);
                    int y1 = (int) Math.round(blitRect.y / scale);
                    int x2 = (int) Math.round((blitRect.x + blitRect.width) / scale);
                    int y2 = (int) Math.round((blitRect.y + blitRect.height) / scale);
                    blitGraphics.drawImage(image, x1, y1, x2, y2, x1, y1, x2, y2, null);
                } finally {
                    blitGraphics.dispose();
                }
                return true;
            }
        }

        private Image wrapLegacyTexture(long legacyPtr) {
            GraphicsConfiguration configuration = graphics.getDeviceConfiguration();
            if (wrappedImage != null && wrappedLegacyPtr == legacyPtr
                    && wrappedConfiguration == configuration) {
                return wrappedImage;
            }
            if (sharedTexturesUnavailable) {
                return null;
            }
            try {
                if (sharedTextures == null) {
                    sharedTextures = new SharedTexturesService();
                }
                Image image = sharedTextures.wrapTexture(configuration, legacyPtr);
                wrappedImage = image;
                wrappedLegacyPtr = legacyPtr;
                wrappedConfiguration = configuration;
                return image;
            } catch (RuntimeException e) { // includes JBRApi.ServiceNotAvailableException
                sharedTexturesUnavailable = true;
                System.err.println("JBR_SKIA_INTEROP_FALLBACK marker=shared-textures-unavailable message="
                        + e.getMessage());
                return null;
            }
        }
        ...
        public boolean renderDiagnosticFrame(int width, int height, long frameTimeNanos) {
            ...
            if (Boolean.getBoolean(NATIVE_DIAGNOSTIC_PROPERTY)
                    && renderNativeAndPresent((target, ox, oy, sx, sy) ->
                            nativeRenderDiagnosticFrame(target, width, height, frameTimeNanos))) {
                return true;
            }
            ...
            try {
                ...
                diagnosticGraphics.drawString("scope=0x" + Long.toHexString(scopeId), 16, Math.min(height - 16, 46));
            } finally {
        ...
        public boolean renderCommandFrame(int width, int height, long frameTimeNanos, int[] commands) {
            ...
            if (NATIVE_BRIDGE_AVAILABLE) {
                ...
                if (renderNativeAndPresent((target, ox, oy, sx, sy) ->
                        nativeRenderCommandFrame(target, ox, oy, sx, sy,
        ...
        public boolean renderCommandBufferFrame(int width, int height, long frameTimeNanos, byte[] commands) {
            ...
            if (NATIVE_BRIDGE_AVAILABLE) {
                ...
                if (renderNativeAndPresent((target, ox, oy, sx, sy) ->
                        nativeRenderCommandBufferFrame(target, ox, oy, sx, sy,
                                width, height, frameTimeNanos, commands))) {
                    return true;
                }
            }
        ...
        public boolean renderCommandDirectFrame(int width, int height, long frameTimeNanos, ByteBuffer commands) {
            ...
            if (NATIVE_BRIDGE_AVAILABLE && commandBuffer.isDirect()) {
                logHandleLifecycleMarkers(decodeCommandBuffer(commandBuffer), contextPtr, "native");
                if (renderNativeAndPresent((target, ox, oy, sx, sy) ->
                        nativeRenderCommandDirectFrame(target, ox, oy, sx, sy,
                                width, height, frameTimeNanos,
                                commandBuffer, commandByteCount))) {
                    return true;
                }
            }
            if (width <= 0 || height <= 0) {
        ...
        public boolean renderPictureFrame(int width, int height, long frameTimeNanos, byte[] pictureData) {
            ...
            return width > 0
                    ...
                    && renderNativeAndPresent((target, ox, oy, sx, sy) ->
                            nativeRenderPictureFrame(target, ox, oy, sx, sy,
        ...
        private static boolean renderJava2DCommands(Graphics2D g, int[] commands, long contextPtr) {
            ...
            try {
                while (offset < commandEnd) {
                    ...
                    if (op == COMMAND_SAVE) {
                    ...
                    } else if (op == COMMAND_STROKE_CLOSED_POLYLINE) {
                    ...
                    } else if (op == COMMAND_SAVE_TRANSLATE_ROTATE_TRANSLATE_FILL_OVAL_RESTORE) {
                        if (offset + 10 != recordEnd) return false;
                        Graphics2D saved = current;
                        Graphics2D transformed = (Graphics2D) current.create();
                        try {
                            ...
                            transformed.translate(commands[offset++] / 1000.0, commands[offset++] / 1000.0);
                            applyAntialiasing(transformed, antiAlias);
                            transformed.setColor(new Color(commands[offset++], true));
                            int x = commands[offset++];
                            int y = commands[offset++];
                            int width = commands[offset++];
                            int height = commands[offset++];
                            transformed.fillOval(x, y, width, height);
                        } finally {
                            transformed.dispose();
                        }
                        current = saved;
                    } else if (op == COMMAND_SAVE_TRANSLATE_ROTATE_TRANSLATE_FILL_OVAL_RESTORE_RUN) {
                        int count = commands[offset++];
                        if (count <= 1 || offset + count * 10 != recordEnd) return false;
                    ...
                    } else if (op == COMMAND_SAVE_TRANSLATE_LAYER_SAVE_TRANSLATE_DRAW_IMAGE_REF_FULL_RESTORE_N) {
                        if (offset + 16 != recordEnd) return false;
                        boolean filtered = (record.recordFlags() & COMMAND_RECORD_FLAG_ANTIALIAS) != 0;
                        double layerDx = commands[offset++] / 1000.0;
                        double layerDy = commands[offset++] / 1000.0;
                        ...
                        drawImage(current, image, filtered, 0, 0, image.getWidth() * 1000, image.getHeight() * 1000,
                                dstLeft1000, dstTop1000, dstRight1000, dstBottom1000, 1000);
                    } else if (op == COMMAND_CLEAR_DRAW_IMAGE_REF_FULL_DRAW_ROUND_RECT) {
                        if (offset + 23 != recordEnd) return false;
                        int clearX = commands[offset++];
                        int clearY = commands[offset++];
                        int clearWidth = commands[offset++];
                        int clearHeight = commands[offset++];
                        if (clearWidth < 0 || clearHeight < 0) return false;
                        int imageFlags = commands[offset++];
                        boolean filtered = (imageFlags & COMMAND_RECORD_FLAG_ANTIALIAS) != 0;
                        int dstLeft1000 = commands[offset++];
                        int dstTop1000 = commands[offset++];
                        ...
                        applyAntialiasing(current, (record.recordFlags() & COMMAND_RECORD_FLAG_ANTIALIAS) != 0);
                        current.setColor(new Color(argb, true));
                        if (paintStyle == COMMAND_PAINT_STYLE_FILL) {
                            current.fill(roundRect);
                        } else {
    ...
    /**
     * Creates or resizes the library-owned shareable replay target; the
     * returned handle supersedes {@code oldTargetHandle} (which is disposed
     * on resize). Returns 0 on failure.
     */
    private static native long nativeAcquireReplayTarget(long oldTargetHandle, int width, int height);

    private static native void nativeDisposeReplayTarget(long targetHandle);

    /** The legacy-shared consumer texture of the target (for SharedTextures). */
    private static native long nativeGetReplayLegacyTexturePtr(long targetHandle);

    private static native boolean nativeRenderDiagnosticFrame(long targetHandle,
                                                              int width, int height, long frameTimeNanos);

    private static native boolean nativeRenderCommandFrame(long targetHandle,
                                                           int destinationX, int destinationY,
                                                           float scaleX, float scaleY,
                                                           int width, int height, long frameTimeNanos,
                                                           int[] commands);

    private static native boolean nativeRenderCommandBufferFrame(long targetHandle,
                                                                 int destinationX, int destinationY,
                                                                 float scaleX, float scaleY,
                                                                 int width, int height, long frameTimeNanos,
                                                                 byte[] commands);

    private static native boolean nativeRenderCommandDirectFrame(long targetHandle,
                                                                 int destinationX, int destinationY,
                                                                 float scaleX, float scaleY,
                                                                 int width, int height, long frameTimeNanos,
                                                                 ByteBuffer commands, int commandByteCount);

    private static native boolean nativeRenderPictureFrame(long targetHandle,
                                                           int destinationX, int destinationY,
                                                           float scaleX, float scaleY,
                                                           int width, int height, long frameTimeNanos,
                                                           byte[] pictureData);
