...
- Latest gradient validation adds live parser-guard rows for stroked gradient command widths. Skiko can corrupt the
  ...
  radial-gradient radius slots to `0`, tile-mode slots to `4`, and color-count slots to `1` across fill/stroke
  ...
  `SKIKO_JBR_INTEROP_RADIAL_GRADIENT*_TILE_MODE_CORRUPTED` / `*_COLOR_COUNT_CORRUPTED` markers, and JBR rejects each
  stream before replay. Focused validation plus the quick `gradient-invalid` group passed:
