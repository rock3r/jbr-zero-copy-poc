...
- Latest RuntimeEffect color-filter hardening adds live SKSL-length, uniform-count, child-count, and named-uniform-count
  sentinels. Skiko can corrupt one recorded RuntimeEffect color-filter descriptor length to `0`, `uniformFloatCount` to
  `257`, `childCount` to `9`, or `namedUniformCount` to `17`; Magic Jewel requires target-specific corruption markers;
  focused and `runtime-effect-invalid` grouped validation passed:
