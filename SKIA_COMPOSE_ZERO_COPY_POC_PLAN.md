...
- Latest RuntimeEffect color-filter hardening adds live SKSL-length, uniform-count, and child-count sentinels. Skiko can
  corrupt one recorded RuntimeEffect color-filter descriptor length to `0`, `uniformFloatCount` to `257`, or
  `childCount` to `9`; Magic Jewel requires target-specific corruption markers; focused and `runtime-effect-invalid`
