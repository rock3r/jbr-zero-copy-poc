...
- JBR now preserves alpha for cached bitmap image imports whose raw pixels contain transparency despite opaque
  `SkPixmap` metadata. This fixes the Jewel showcase Buttons icon-background regression in command replay. The exact
  ...
  stayed strict-clean and now emits both old/new screenshots. Visual inspection confirms the selected left-rail Buttons
  item matches the baseline while the command-path body icon transparency guardrail still holds.
...
- Magic Jewel's IDE benchmark suite now preserves old/new Spectre toolwindow crops separately, samples per-thread CPU,
  ...
  showing worse current short-run process/thread/RSS numbers for the zero-copy path than the old path:
...
- JBR/CMP/Skiko now support compact `saveLayer; save; translate` records through ABI 111
  `COMMAND_SAVE_LAYER_SAVE_TRANSLATE` (`op=93`). Focused Markdown wheel validation stayed strict-clean with zero
...
- JBR/CMP/Skiko now support compact `fillRect; saveLayer; clipRect` records through ABI 111
...
- JBR/CMP/Skiko now support stroked descriptor-backed shader rects and stroked image-shader rects. Magic Jewel's exact
  descriptor stroke shader row passed with `fallback_new_count=0`, `unsupported=none`, zero picture frames, and 1,728
  command frames:
  `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-command-probe-suite/20260618-171051/suite.tsv`.
- CMP now records image draws with path effects through the existing image-ref command path. Magic Jewel's exact image
  path-effect row passed with `fallback_new_count=0`, `unsupported=none`, zero picture frames, and 1,245 command
  frames:
  `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-command-probe-suite/20260618-171756/suite.tsv`.
- CMP now accepts image command dimensions up to JBR's existing 4096-pixel cap. Magic Jewel's exact 2049-wide image
  shader row passed with `fallback_new_count=0`, `unsupported=none`, zero picture frames, and 1,589 command frames:
  `/Users/rock3r/src/jbr-skia-zero-copy/magic-jewel/out/jbr-skia-command-probe-suite/20260618-172205/suite.tsv`.
- Magic Jewel promoted six raw-pointer shader fixtures to descriptor-backed Compose shader constructors so no raw
...
- Magic Jewel promoted the vertices raw color-filter fixture and four remaining raw/opaque/picture shader fixtures to
  descriptor-backed command paths. The five-row focused command probe passed with `fallback_new_count=0`,
  `unsupported=none`, zero picture frames, and non-zero command frames:
...
- A targeted read-only graphics-layer resize/forced-context lifecycle gap audit found no remaining scoped lifecycle
  ...
  not run a broad validation suite.
